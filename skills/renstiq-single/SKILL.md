---
name: renstiq-single
description: renstiqの単一repoルールでPR候補を取得し、先頭一致で選ばれた指示に沿ってレビュー・マージ・後処理を行う。調査依頼では操作しない。
---

# 単一repoのPR処理

依頼されたrepo・PR・実行場所・操作範囲を守る。`REPO_DIR` はローカルのrepoルート、`OWNER/REPO` はCLIが返すGitHub名。処理ポリシーは対象repoの `renstiq.yaml` だけを使う。`--config` は探索用で、これらのrepoコマンドには渡さない。
...
ルールの先頭一致判定と候補選別はCLIに任せ、`selection: candidate` のPRだけを処理する。`config show` はマージ方法と保留時・後処理の設定を読むために使う。開始時の件数・PR番号・head/base SHAを記録する。設定不正や無効化は報告し、依頼なく設定を変更しない。

作者は各 `rules` の `authors` で決まり、AIがRenovate限定などの条件を足さない。通常は候補だけを取得する。依頼されたPRが一覧にない理由など、除外・判定不能の調査が必要なときだけ `pr list --all` を使い、出力から必要なPRと理由だけを読む。`open_pr_count` は作者・フィルタ条件で絞る前のopen PR総数。

`complete: false`、`errors`、非0終了を報告しつつ、取得できた候補の処理は続ける。`unknown` は候補に加えず、件数の `null` は不明として扱う。

## 候補をレビューする

選ばれたルールの指示に必要な詳細を、`gh pr view`、`gh pr diff`、`gh pr checks`、`gh api` などで取得する。

`pr list` のcandidateには、上から最初に一致したルールの指示が `review` 配列の1件として返る。その `instructions` を実施し、結果を `id` ごとに記録する。後ろのルールの指示を追加したり、本文を合成したり、条件を再判定しない。candidateなのにreviewが欠ける・0件・複数件の場合は、CLIと設定の整合性を確認できるまで処理を保留する。どのルールにも一致しないPRはexcludedで、先のルールが判定不能なら後ろへ飛ばさずunknownになる。
...
`match`・`exclude` があれば、変更ファイル・依存名・更新種別に照らして対象を確認する。依存名と更新種別は同じ更新で評価し、matchした更新からexcludeした後に1つ以上残れば適用する。空のmatchは制限なし、空のexcludeは除外なし。`after_repo` はマージしたPRごとに評価し、別PRの情報を混ぜない。項目の仕様が必要なら `renstiq schema repo` で確認する。
